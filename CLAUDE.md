...
- Anti-banimento: throttle 1 msg/1.2s + jitter ≤800ms. Campanha 1 msg/5s. Spinning de copy. Janela 6h-23h, domingo aberto por padrão (opcional por conexão — alguns modelos de negócio atendem domingo, outros preferem fechar)
  - **Cortesia não é anti-ban** (`docs/doctrine/restricao-de-canal.md`, invariante 3): a janela 7h-22h/sem-domingo/fuso do tenant existe para **não incomodar o cliente** e vale em **todo** canal, mesmo sem risco de ban — nunca é desarmada junto com throttle/jitter/warm-up/cap diário, que são anti-ban de verdade e só armam onde há risco (`decidePacing`, `lib/agent-engine/pacing/engine.ts`, parâmetro `banRisk`). Um agente "24h" não remove essa janela — quem quiser horário estendido para um número específico usa o override por conexão em Conexões → proteção anti-ban (`channel_knobs`/`AntiBanSheet.tsx`), ciente de que também passa a poder escrever ao cliente de madrugada
...
| `docs/prd/02-...05-` | Customer 360, WhatsApp, Pipeline, IA-RAG |
