...
| J5.10-J5.14 | ~~Licenciamento self-host por instalação (PaySuite): `/app/settings/billing`, gate `403 license_required` em `proxy.ts`, `/admin/licensing` (credencial + emissão de licença)~~ | **Removido por completo (decisão do dono do produto, 2026-09-09).** O produto pivotou de licenciamento por instalação self-host para instância central multi-tenant (`docs/superpowers/specs/2026-09-09-licenciamento-por-tenant-design.md`) — painel `/admin/licensing`, gate global de mutação, cron de renovação e a tela `/app/settings/billing` foram excluídos do código (commit `ca8c9495`). O modelo atual é licenciamento por **tenant** dentro da instância central — criação de tenant + convite do owner tem E2E próprio (`tests/e2e/admin-cria-tenant-convite-owner.spec.ts`), ver `docs/superpowers/plans/2026-09-09-licenciamento-por-tenant.md`. |
...
| `M9` `[P0]` | **Logo por arquivo**: o dono do servidor sobe um PNG e ele aparece na barra lateral E no `/login` **deslogado**; a empresa sobe o dela e a fachada NÃO muda; SVG renomeado é recusado com a razão; remover devolve o logo da camada de baixo | **SPEC ESCRITA, EXECUÇÃO PENDENTE POR INFRA.** `tests/e2e/marca-logo.spec.ts` no disco e inscrita em `SPECS_PARTE_2` (`.github/workflows/e2e.yml:148`, como ÚLTIMA da lista — se a restauração dela falhar, o alcance da contaminação é zero spec), com os 6 casos e as medições por ferramenta (`src` + `naturalWidth`). ⚠️ **A régua desta linha já esteve errada:** ela dizia `getBoundingClientRect().height`, "porque `src` certo com altura 0 é o sintoma de bucket privado". É falso — os `<img>` de marca têm altura fixada por CSS (`h-7`, `h-10`), e o medido em chromium é `boa={"nat":1,"altura":28}` contra `quebrada={"nat":0,"altura":28}`: a altura passava nos dois. Quem prova o download é `naturalWidth`. **MEDIDO em 2026-09-22 (commit `abe7dfbfe`, job `e2e-parte (2)` do CI): passou, os 6 casos.** A linha acima ("NÃO MEDIDO") valia até então — a CI ficou bloqueada por billing do GitHub Actions desde antes desta spec existir, então nunca tinha rodado de verdade (nem o Docker local citado embaixo resolvia isso: o gate é o CI, não a máquina de quem escreveu). Na primeira execução real (commit `97cbe98b4`) o caso (4) ("SVG renomeado... recusado pelos BYTES") falhou por uma condição de corrida no PRÓPRIO teste — `setInputFiles` despachando o evento nativo antes do React hidratar e anexar o `onChange` de `CampoDeLogo`, medido por trace (zero requisições de rede no período inteiro de espera do toast). Não era bug de produto. Endurecido em `b756fa504`: `subir()` agora confirma com `waitForResponse` que a rede reagiu antes de seguir, com retry. O que ESTÁ medido também é o lado unitário (`tests/unit/branding-logo-arquivo.test.ts`, 19 casos, com 3 sabotagens de contagem prevista) e a estrutura do banco (`tests/invariants/marca-logo.test.ts`, escrito e **não executado** neste ciclo — cobre schema, não fluxo de tela) |
...
| `J10.5` | **Endereço de suporte** — o cliente do revendedor nunca vê o nosso | `SUPPORT_EMAIL` (resolvido em `lib/branding/saida.ts:238`) aparece em `/account-suspended`; sem ele, o parágrafo some em vez de mostrar um endereço nosso. (`/app/settings/billing` também aparecia aqui — a rota foi removida por completo em 2026-09-09, ver J5.10-J5.14 acima.) | pela tela |
