...
| J5.13 | **Central:** platform admin abre `/admin/licensing` e cola token+webhook secret do PaySuite | credencial cifrada em `licensing_paysuite_credentials` (nunca reexibida); `/api/v1/licensing/renew` e o webhook de confirmação passam a usar essa credencial, não mais env var — ainda **sem prova de tela** |
| J5.14 | **Central:** platform admin preenche o formulário "Nova licença" (nome, email, valor, dias de teste) | chave da licença aparece uma vez na tela pra copiar e enviar ao cliente; a licença some da lista se recarregar sem copiar? **Não** — a lista (`GET /api/v1/licensing/admin`) sempre mostra a chave de novo, decisão deliberada (a Task 1 já guarda `license_key` em texto no banco, não hash) — ainda **sem prova de tela** |
